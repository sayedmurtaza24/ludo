...
pub const Dice = struct {
    ...
    pub fn rollWithNum(self: *Self, num: u3) error{NotRollable}!u3 {
...
pub const Game = struct {
    ...
    pub fn move(self: *Self, piece: Piece) !void {
        ...
        const diceNum = try self.dice.setUsed();
    ...
    pub fn forward(self: *Self) !void {
        ...
        s: switch (self.dice.mod) {
            .NotRolled => return error.DiceNotRolled,
            .Rolled => {
                const moves = self.availableMoves(self.curr).count();
                if (moves != 0) return error.DiceRolledNotUsed;
                _ = self.dice.setUsed() catch {};
                continue :s .Used;
            },
            .Used => self.dice.reset() catch {},
        }
